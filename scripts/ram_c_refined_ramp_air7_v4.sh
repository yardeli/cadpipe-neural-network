...
% v4: CFL=0.5 (matches AIR-5 working settings) with adapt range 0.1-2.0.
% v3 cold-start at CFL=0.2 hit a residual limit cycle around -0.79 once
% chemistry stabilized — too dissipative to push through bulk shock
% adjustment. With chemistry already in equilibrium, higher CFL is safe.
CFL_NUMBER= 0.5
CFL_ADAPT= YES
CFL_ADAPT_PARAM= ( 0.5, 1.5, 0.1, 2.0 )
ITER= $iter
...
% Bulk-flow residual. Engineering target -2 (factor-100 reduction); -3 was
% v3's overspec given the limit-cycle behavior of cold-start.
...
run_stage() {
    ...
    local restart=$3
    local prev_dir=$4
    local stage_dir="${RUNS}/ramC_refined_air7v4_M${mach/./_}_A61"
    mkdir -p "$stage_dir"
...
run_stage 15.0 1500 YES "${RUNS}/ramC_refined_air7v4_M10_0_A61"
run_stage 18.0 1500 YES "${RUNS}/ramC_refined_air7v4_M15_0_A61"
run_stage 22.5 3000 YES "${RUNS}/ramC_refined_air7v4_M18_0_A61"

echo "======================================================"
echo "  AIR-7 v3 (cold-start) refined-mesh ramp complete."
echo "  M22.5 flow.vtu at:"
echo "    ${RUNS}/ramC_refined_air7v4_M22_5_A61/flow.vtu"
echo "======================================================"
