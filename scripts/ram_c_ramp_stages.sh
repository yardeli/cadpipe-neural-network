#!/bin/bash
# Chain the RAM-C Mach ramping on the small tagged mesh.
#
# Assumes Stage 1 (M10) has completed successfully and produced
# restart.dat in /home/yarden/ram_c_runs/ramC_small_M10_A61/.
#
# Runs sequentially: M15 → M18 → M22.5, using each stage's restart
# as the next stage's initial condition. Each stage waits for the
# previous to produce restart.dat before launching.
#
# Usage on VM:
#   bash ram_c_ramp_stages.sh

set -e

RUNS=/home/yarden/ram_c_runs
MESH=ram_c_tagged.su2
...
run_stage() {
    ...
    local iter=$2
    local prev_dir=$3
    local stage_dir="${RUNS}/ramC_small_M${mach/./_}_A61"
    mkdir -p "$stage_dir"

    # Copy mesh
    [ -f "$stage_dir/$MESH" ] || cp "${RUNS}/ramC_small_M10_A61/$MESH" "$stage_dir/"
    # Copy restart as solution.dat (SU2 restart convention)
    ...
    echo "Stage M=$mach DONE — final Rho_0 residual = $final_res"
    echo ""
}

# Stage 2: M15 from M10 restart
run_stage 15.0 400 "${RUNS}/ramC_small_M10_A61"

# Stage 3: M18 from M15 restart
run_stage 18.0 400 "${RUNS}/ramC_small_M15_0_A61"

# Stage 4: M22.5 from M18 restart
run_stage 22.5 600 "${RUNS}/ramC_small_M18_0_A61"

echo "======================================================"
echo "  Ramp complete. Final M22.5 flow.vtu at:"
echo "    ${RUNS}/ramC_small_M22_5_A61/flow.vtu"
echo "======================================================"
