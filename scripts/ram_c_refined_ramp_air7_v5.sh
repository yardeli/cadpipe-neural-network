...
% v5: middle-ground CFL=0.3 + looser linear solver. v3 (CFL=0.2) hit limit
% cycle around RhoU=-0.79; v4 (CFL=0.5) hung in BCGSTAB on iter 1 (linear
% system too ill-conditioned at higher CFL). v5 widens BCGSTAB iters from
% 5→15 and loosens tolerance from 1e-6→1e-4 — accepts less accurate inner
% solve, but iter completes instead of hanging.
CFL_NUMBER= 0.3
CFL_ADAPT= YES
CFL_ADAPT_PARAM= ( 0.5, 1.5, 0.1, 1.0 )
ITER= $iter
...
% Bulk-flow residual; -2 engineering target (cold-start can't reach -3
% without long limit-cycle settling).
...
run_stage() {
    ...
    local restart=$3
    local prev_dir=$4
    local stage_dir="${RUNS}/ramC_refined_air7v5_M${mach/./_}_A61"
    mkdir -p "$stage_dir"
...
# Cold-start at M10, then chain through ramp.
run_stage 10.0 1500 NO ""
run_stage 15.0 1500 YES "${RUNS}/ramC_refined_air7v5_M10_0_A61"
run_stage 18.0 1500 YES "${RUNS}/ramC_refined_air7v5_M15_0_A61"
run_stage 22.5 3000 YES "${RUNS}/ramC_refined_air7v5_M18_0_A61"

echo "======================================================"
echo "  AIR-7 v3 (cold-start) refined-mesh ramp complete."
echo "  M22.5 flow.vtu at:"
echo "    ${RUNS}/ramC_refined_air7v5_M22_5_A61/flow.vtu"
echo "======================================================"
