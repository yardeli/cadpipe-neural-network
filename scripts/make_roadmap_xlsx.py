...
TASKS = [
    ...
    ("F-1", "AIR-11 chemistry ramp — primary fix for downstream plasma collapse", "Validation Fix", "In Progress",
     30, "Day 8 (overnight)", "2026-04-25", "2026-04-25 ETA +12h", "C-3a",
     "Refined-mesh AIR-5 result has log10 err -1.59 vs J&C 1972. Sheath-decay diagnostic (74226ef) shows axial decay -19.4 log10/zL — chemistry collapses 5 orders over 45cm of body. AIR-5 + Saha post-process doesn't carry ions through flow; AIR-11 transports e-, N+, O+, NO+, N2+, O2+ as proper species. Expected -0.6 to -1.0 log10. Running on VM PID 138300, ETA ~12h. Monitor task b3sgwr33p hourly. Script: scripts/ram_c_refined_ramp_air11.sh."),
    ("F-2", "AIR-11 + non-catalytic Navier-Stokes combined ramp (fallback)", "Validation Fix", "Pending",
     0, "Day 9", "—", "—", "F-1",
     "Pre-staged at scripts/ram_c_refined_ramp_catalysis.sh (commit 8d807aa). Run only if F-1 leaves |err| > 1.0. SOLVER= NEMO_NAVIER_STOKES + MARKER_ISOTHERMAL=( body, 2500 ) + MARKER_CATALYTIC=( body, NON_CATALYTIC ). Wall catalysis acts in BL — keeps radicals from recombining at body, sustains downstream ne. Expected -0.9 to -1.4 log10 (synergistic). ~24h wall time on VM."),
    ("F-3", "Anisotropic near-wall mesh refinement (BL layers)", "Validation Fix", "Pending",
     0, "Day 10-11", "—", "—", "F-1",
     "BL HWHM averages 1.67 cm (74226ef diagnostic), only 4 cells across at 4mm body mesh. Marginal. Going to 0.5-1mm at wall via gmsh BoundaryLayer field gives 16-30 cells. Isotropic 1mm is 30M cells (impossible); anisotropic keeps tangential 4mm but compresses normal-to-wall to 0.5mm in geometric layers. Expected -0.2 to -0.4 log10. Third-leg fix only if F-1+F-2 don't reach |err| < 0.5."),
    ("F-4", "Tight-convergence rerun of existing M22.5 (cheap polish)", "Validation Fix", "Pending",
     0, "Day 8 (cheap, opportunistic)", "—", "—", "C-3a",
     "Existing M22.5 plateaued at Rho_0 = -2.10. Conventional CFD target is -3 to -4. Push existing restart further at CFL=0.3 for 1500-2000 more iters. Expected -0.1 to -0.2 log10. Cheapest available improvement, ~3h wall time. Only run if final |err| is in 0.4-0.7 range and we want 'EXCELLENT' verdict (|err| < 0.3). Otherwise skip."),
    ("F-5", "T_wall sensitivity sweep (1500/2000/2500/3000 K)", "Validation Fix", "Pending",
     0, "Day 11-12", "—", "—", "F-2",
     "Catalysis ramp uses T_wall=2500K as RAM-C SiO2 ablation temp. Real value varies 1500-3000K. Sweep 4 values, M22.5 stage only, starting from F-2's M18 restart. Quantifies T_wall as a calibration knob. Expected 0 to -0.2 log10. ~16h wall time on VM."),
    ("F-6", "ParaView 3D diagnostic — visualize plasma collapse geometrically", "Validation Fix", "Pending",
     0, "Day 8 (any time)", "—", "—", "C-3a",
     "Open data/checkpoints/ramp2_M22_5_refined/flow.vtu in ParaView, apply Threshold + Isosurface + Slice filters on n_e. Renders the plasma envelope in 3D — can't see this from 2D matplotlib scatter. Informational only (no log10 reduction). Paper-grade visuals. ~15 min ParaView GUI work."),
    ("F-7", "Direct LOS attenuation comparison vs J&C published dB (HIGH PRIORITY)", "Validation Fix", "Pending",
     0, "Day 8 (NOW — parallel with F-1)", "—", "—", "—",
     "Headline metric we're chasing (log10 ne) may be the wrong goalpost — J&C reflectometer measured TOTAL ATTENUATION in dB, not ne. Look up published dB values for VHF 225/450 + X-band 9.2 GHz at 61 km (Jones & Cross 1972 Table III). Add as reference data in plasmanet/ram_c_validation.py. Update validate_ram_c_nemo.py to compute log10 dB error alongside log10 ne error. If integrated dB matches within ~20%, we can claim engineering accuracy WITHOUT closing the ne gap. Could be the actual answer. ~30 min effort. NO new CFD needed."),
