...
TASKS = [
    ...
    ("S-1", "Mechanism generator — programmatic SU2 GAS_MODEL/Cantera config emission", "Search Framework", "In Progress",
     50, "Week 1", "2026-04-25", "—", "—",
     "plasmanet/mechanism_search/generator.py written: Reaction + Mechanism dataclasses, subset() filter API, Cantera YAML emitter, SU2 cfg snippet emitter. Park 47 mechanism scaffolded with 18/47 reactions filled (dissociation + exchange + associative ionization + DR + charge-transfer). 19-47 are placeholders (need extraction from Park 1990 Tables 2+4+6). Includes park_air5/air7/air11 helpers and random_subset() for search."),
    ("S-2", "Fast evaluator — Cantera 0D + boundary corrections surrogate", "Search Framework", "In Progress",
     40, "Week 1-2", "2026-04-25", "—", "S-1",
     "plasmanet/mechanism_search/cantera_evaluator.py written: normal-shock with chemistry energy-sink correction → Cantera IdealGasConstPressureReactor → ne extraction → Appleton-Hartree dB attenuation. Auto-registers as 'cantera_0d' evaluator with the scoring module. Local Cantera not installed (Windows wheel build fails); needs install on VM via pip install cantera. Validation against AIR-5/AIR-7 CFD baselines pending."),
    ...
    ("S-6", "Comparison framework — mechanism scoring vs published data", "Search Framework", "In Progress",
     80, "Week 1-2", "2026-04-25", "—", "—",
     "plasmanet/mechanism_search/scoring.py: BenchmarkCondition + 4 published RAM-C points (61/71/81/47 km from J&C 1972 + Grantham 1970, weighted 2.0 for primary anchor). score_against_benchmark() computes log10 ne error + dB margin per band. score_candidate() top-level dispatch with pluggable evaluator backends. CFD-VTU evaluator built-in (auto-registers). Smoke-tested: AIR-5 prediction reproduces our measured log10 err = -1.59. Just need FIRE-II + Apollo benchmarks added for full coverage."),
