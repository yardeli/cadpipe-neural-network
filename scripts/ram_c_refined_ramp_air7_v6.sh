...
% Conservative CFL for cold-start. AIR-5 worked at 0.5; reduced for ions.
CFL_NUMBER= 0.2
CFL_ADAPT= YES
CFL_ADAPT_PARAM= ( 0.5, 1.5, 0.05, 1.0 )
ITER= $iter
...
% v6: revert to v3 proven settings (CFL=0.2, BCGSTAB iter=5, tol=1e-6)
% but with -2 convergence target. v3 hit limit cycle ~-0.79 with -3 target;
% v4 (CFL=0.5) and v5 (CFL=0.3 + iter=15) both hung in BCGSTAB on iter 1.
% v6 accepts the -0.79 limit cycle as engineering-acceptable steady state.
...
run_stage() {
    ...
    local restart=$3
    local prev_dir=$4
    local stage_dir="${RUNS}/ramC_refined_air7v6_M${mach/./_}_A61"
    mkdir -p "$stage_dir"
...
run_stage 15.0 300 YES "${RUNS}/ramC_refined_air7v6_M10_0_A61"
run_stage 18.0 300 YES "${RUNS}/ramC_refined_air7v6_M15_0_A61"
run_stage 22.5 800 YES "${RUNS}/ramC_refined_air7v6_M18_0_A61"

echo "======================================================"
echo "  AIR-7 v3 (cold-start) refined-mesh ramp complete."
echo "  M22.5 flow.vtu at:"
echo "    ${RUNS}/ramC_refined_air7v6_M22_5_A61/flow.vtu"
echo "======================================================"
